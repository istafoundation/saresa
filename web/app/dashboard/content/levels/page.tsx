...
`;
  ...
  return (
    <div className="space-y-6">
      ...
      {showMoveLevelModal && (
        <MoveLevelModal
          level={showMoveLevelModal}
          groups={groups}
          onClose={() => setShowMoveLevelModal(null)}
          onMove={async (groupId) => {
             await updateLevel({
               levelId: showMoveLevelModal._id,
               groupId: groupId
             });
             setShowMoveLevelModal(null);
          }}
        />
      )}
...
function MoveLevelModal({ level, groups, onClose, onMove }: {
    level: Level;
    groups?: Doc<"levelGroups">[];
    onClose: () => void;
    onMove: (groupId?: Id<"levelGroups">) => Promise<void>;
}) {
    const [groupId, setGroupId] = useState<string>(level.groupId ?? "");
    const [isSubmitting, setIsSubmitting] = useState(false);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        setIsSubmitting(true);
        await onMove(groupId ? groupId as Id<"levelGroups"> : undefined);
        setIsSubmitting(false);
    };

    return (
        <Modal title={`Move Level: ${level.name}`} onClose={onClose}>
            <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">Select Group (World)</label>
                    <select
                        value={groupId}
                        onChange={(e) => setGroupId(e.target.value)}
                        className="w-full px-4 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500"
                        size={5} // Show multiple options/list style
                    >
                        <option value="">(No Group / Ungrouped)</option>
                        {groups?.map((g) => (
                            <option key={g._id} value={g._id}>
                                {g.theme?.emoji} {g.name}
                            </option>
                        ))}
                    </select>
                </div>
                <div className="flex justify-end gap-2">
                    ...
                    <button
                        ...
                        className="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 disabled:opacity-50"
                    >
                        {isSubmitting ? "Moving..." : "Move Level"}
                    </button>
                </div>
            </form>
        </Modal>
